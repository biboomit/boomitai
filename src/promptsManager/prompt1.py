class Prompt1:
   def __init__(self):
      ...
      self.descripcion="""Analiza los costos diarios de un conjunto de datos publicitarios a nivel de plataforma y proporciona puntos de interés. 
...
"""
      ...
      self.prompt = [
         ...
         [
            [
               "La variación porcentual del costo medio entre períodos",
               "La magnitud del cambio (significativo, moderado o estable)",
               "El contraste con la variación en la inversión"
            ],
            [
      ...
      self.subTitles = [
         ...
         [],
         [
            "- Calculos para el periodo anterior (especificar fechas):",
            "- Calculos para el periodo actual (especificar fechas):",
      ...
      # EXAMPLE
      self.example = """Ejemplo de respuesta:
...
Se recomienda continuar monitoreando las estrategias de inversión en Meta y Google Ads, dado el aumento en la eficiencia de costos. Para TikTok Ads, aunque la eficiencia ha mejorado, sería prudente investigar formas de optimizar aún más la inversión para mantener esta tendencia positiva."""
      ...
      if paso == 0 or paso == 1: #paso1 y paso2
         ...
         if '1' in key:
            ...
            for i in range(len(key)):
               if key[i] == '1':
                  prompt += self.whitespace * 2
                  prompt += chr(96 + (i + 1)) + ") " + self.prompt[paso][index][i] + "\n"
      elif paso == 4: #paso5
         if '1' in key:
            prompt += self.whitespace
   ...
   def obtainExample(self, key, paso):
      return self.example
